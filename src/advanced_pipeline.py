...
# classify scene based on brightness and fog density
@time_function
def classify_scene(frame_gray):
    ...
    return lighting, is_foggy, brightness, fog_index
...
def adaptive_equalize(roi_gray, lighting):
    if lighting == 'night':
        return CLAHE_NIGHT.apply(roi_gray)
    elif lighting == 'dusk':
        return CLAHE_DUSK.apply(roi_gray)
    else:
        return cv2.equalizeHist(roi_gray)

# Get detection parameters suited for current scene
def get_adaptive_params(lighting, is_foggy):
    if is_foggy:
...
@time_function
def process_single_roi(roi_f1, roi_f2, grid_w, grid_h, lighting, is_foggy):
    ...
    params = get_adaptive_params(lighting, is_foggy)
    return process_grid_vectorized(ch_data, grid_w, grid_h, params)

# Worker function for parallel processing
def worker_task(args):
    frame_idx, r1f1, r1f2, r2f1, r2f2, lighting, is_foggy = args

    mat1 = process_single_roi(r1f1, r1f2, GRID_W1, GRID_H1, lighting, is_foggy)
    mat2 = process_single_roi(r2f1, r2f2, GRID_W2, GRID_H2, lighting, is_foggy)
...
@time_function
def _equalize_frame_rois_adaptive(frame, lighting):
    """Apply scene-adaptive equalization to frame ROIs for visualization."""
...
def batch_roi_generator(video_source, batch_size):
    ...
    if is_seq:
        ...
        lighting, is_foggy, _, _ = classify_scene(gray_scene)
        ...
        for i in range(1, len(files)):
            ...
            batch.append((i, prev_rois[0], curr_rois[0], prev_rois[1], curr_rois[1], prev, lighting, is_foggy))
        ...
        while True:
            ...
            batch.append((idx, prev_rois[0], curr_rois[0], prev_rois[1], curr_rois[1], prev, lighting, is_foggy))
...
@time_function
def draw_scene_info(frame, lighting, is_foggy, brightness, fog_idx):
    condition = lighting.upper() + (" + FOG" if is_foggy else "")
...
def main():
    ...
    writer = AsyncVideoWriter('output/videos/output_advanced_cv.mp4', fourcc, fps, (w, h))
    ...
    with Pool(processes=n_procs) as pool:
        ...
        for batch in prefetch_batches(gen):
            ...
            worker_args = [(idx, r1f1, r1f2, r2f1, r2f2, lighting, foggy) for idx, r1f1, r1f2, r2f1, r2f2, _, lighting, foggy in batch]
            original_frames = {idx: frame for idx, _, _, _, _, frame, _, _ in batch}
